...
- peer-to-peer 유저가 언제든지 모든 종류의 데이터를 주고 받을 수 있는 채널이다.

- 비디오와 오디오 뿐만 아니라 이미지, 파일, 텍스트, 게임 업데이트 패킷 같은 것들도 서로 주고받을 수 있게 된다.

- Socket.IO 없이도 채팅을 만들 수 있다. Web Socket을 더이상 만들 필요가 없다. 채팅을 만들면 메세지가 peer-to-peer로 전달이 된다. 

---

### 2. WebRTC의 문제점

- peer 수가 늘어나면 느려지기 시작할 것이다.

- 같은 video stream을 각 peer에게 여러번 업로드 한다. 또한 여러번 다운로드 받아야 한다. 이 때 네트워크가 느려질 것이다. 

- 결과적으로 다수가 참가하는 회의에서는 webRTC는 바람직하지 않다.

---

### 3. SFU

- Selective Forwarding Unit의 약자로 서버에 의존한다.

- 모두가 스트림을 업로드하고 다운로드 하고 있다. 

- 이 서버는 모두로부터 스트림을 받는데, 이 서버는 그 스트림들을 압축하고 있다.

- 그리고 이 서버는 누가 이 방의 호스트인지, 누가 말하고 있는지, 누가 스크린을 공유하고 있는지, 누가 발표를 하고 있는지를 알고 있다.

- 즉, 듣고 있기만 한다면 그 사용자 화면은 존재하지만 작게 있을 것이다.

- 그 때 이 서버는 사용자의 스트림을 압축할 것이다.

- 그래서 사람들이 이 스트림을 다운로드할 때 질이 좀 안 좋은 스트림을 다운로드 하게된다.

- 우리가 서버에 업로드하면, 서버는 다른 사람들에게 저사양의 스트림을 제공한다. 누가 말하고 있느냐에 따라서 다를 수도 있다.

- 스크린을 공유하고 있거나 발표하고 있는 사람의 화면이라면 서버는 좀 더 좋은 사양으로 제공할 것이다.
