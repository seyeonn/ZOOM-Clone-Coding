...
- 사용자와 개발자의 편의를 위한 개선사항
    ...
    1. 지금은 메세지를 보낼 때 메세지를 보낸 후, 값을 비워 주고 있다. 그리고 그 메세지는 서버로 전송된다. 이 말은 즉슨, edge가 메세지를 보내면, edge에게도 메세지를 보내주는 것이다. (같은 메세지를 보낸다는 말이다.) **나를 제외하고 다른 사람 모두에게 메세지를 보내는 방법을 찾아보자.**

- 다음 섹션에서 socket을 다루는 프레임워크를 배워보자.
